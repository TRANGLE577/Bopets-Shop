	...
	<ul class="pagination justify-content-end">
		<c:if test="${!pagedListHolder.firstPage}">
			<li class="prev page-item"><a class="page-link"
				href="<%=StringUtils.replace(pagedLink, "~", String.valueOf(pagedListHolder.getPage() - 1))%>"><ion-icon name="arrow-back-outline"></ion-icon></a></li>
		</c:if>
		...
		<c:if test="${!pagedListHolder.lastPage}">
			<li class="next page-item"><a class="page-link"
				href="<%=StringUtils.replace(pagedLink, "~", String.valueOf(pagedListHolder.getPage() + 1))%>"><ion-icon name="arrow-forward-outline"></ion-icon></a></li>
		</c:if>
	</ul>
</c:if>